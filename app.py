...
model4=pickle.load(open('forest.pkl','rb'))
@app.route("/")
...
def ValuePredictor(to_predict_list, size):
    ...
    if(size==8):#Diabetes
        ...
        loaded_model = joblib.load("glass")
        result =loaded_model.predict(to_predict)
    return result[0]
...
def result():
    ...
    if(flag==0):
        if(int(result)==1):
            prediction='Sorry!!! Looks like you are Suffering!'
        else:
            prediction='Hurray!!! You are Healthy for now. Take Care!' 
    elif(flag==1):
        if(int(result)==0):
            prediction='The variety of Iris is Setosa!'
        elif(int(result)==1):
            prediction='The variety of Iris is Versicolor!'
        else:
            prediction='The variety of Iris is Virginica!'
    else:
        if(int(result)==1):
...
def predict():
    ...
    if output>str(0.5):
        prediction='The Forest is in Danger.\nProbability of fire occuring is {}'.format(output)
    else:
        prediction='The Forest is safe.\n Probability of fire occuring is {}'.format(output)
    return(render_template("result.html", prediction=prediction))

@app.route('/predict_em',methods=['POST'])
def predict_em():
    
    return render_template('result.html', prediction=predict.prediction)
